...
export default function ListOfUser() {
    const users2 = useAppSelector((state) => state.users)
                ...
                <TableBody>
                    {users2.map((item) => (
                        <TableRow key={item.id}>
                            ...
                            <TableCell>
                                <img style={{ width: "32px" }} src={`https://unavatar.io/github/${item.github}`} />
                            </TableCell>
                            <TableCell>{item.name}</TableCell>
                            <TableCell>{item.email}</TableCell>
                            <TableCell>
                                <button onClick={() => { handleRemoveUser(item.id) }} className='mr-2'>
